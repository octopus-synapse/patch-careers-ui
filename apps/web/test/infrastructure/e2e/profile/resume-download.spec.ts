...
test.describe('Resume PDF Download', () => {
	...
	test('browser download button works on profile page', async ({ browser }) => {
		...
		if (!btnVisible) {
			console.log('Download button not visible. Console:', consoleMessages.filter(m => m.includes('error') || m.includes('Error')));
		...
		if (errorText) {
			console.log('Download error visible. Console errors:', consoleMessages.filter(m => m.includes('error') || m.includes('Error')));
		}
