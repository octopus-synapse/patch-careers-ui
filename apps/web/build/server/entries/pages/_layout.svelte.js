...
function Nav_logo($$renderer, $$props) {
  let { textClass } = $$props;
  $$renderer.push(`<a href="/"${attr_class(`text-sm font-bold tracking-tight ${stringify(textClass)}`)}>patch<span class="font-light opacity-40">careers</span></a>`);
}
...
function Segment_toggle($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div${attr_class(`flex rounded-md p-0.5 ${stringify(bg[colorSchema2])}`)}><!--[-->`);
    const each_array = ensure_array_like(options);
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let option = each_array[$$index];
      $$renderer2.push(`<button${attr_class(`rounded font-semibold transition-all ${stringify(sizeClass[size])} ${stringify(selected === option.value ? active[colorSchema2] + " " + text[colorSchema2] : muted[colorSchema2])}`)}>${escape_html(option.label)}</button>`);
    }
    $$renderer2.push(`<!--]--></div>`);
  });
}
function Nav_user_dropdown($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let {
      user,
      cs,
    ...
    $$renderer2.push(`<div class="relative hidden md:block" data-dropdown=""><button class="flex items-center gap-2 p-1">`);
    Avatar($$renderer2, { name: initial(), colorSchema: cs, size: "sm" });
    $$renderer2.push(`<!----> `);
    ...
    if (isOpen2) {
      ...
      if (cs === "dark") {
        ...
        $$renderer2.push("<!--[-1-->");
        Moon($$renderer2, { size: 13 });
      }
      $$renderer2.push(`<!--]--> <span class="text-[10px] font-semibold uppercase tracking-widest">${escape_html(themeLabel)}</span></div> `);
      Segment_toggle($$renderer2, {
        options: themeOptions,
        selected: cs,
        colorSchema: cs,
        onchange: onthemetoggle
      });
      $$renderer2.push(`<!----></div></div> <div class="px-4 py-3"><div class="flex items-center justify-between"><div${attr_class(`flex items-center gap-2 ${stringify(s.muted[cs])}`)}>`);
      Globe($$renderer2, { size: 13 });
      $$renderer2.push(`<!----> <span class="text-[10px] font-semibold uppercase tracking-widest">Language</span></div> `);
      Segment_toggle($$renderer2, {
      ...
      $$renderer2.push(`<!----></div></div></div> <div${attr_class(`border-t ${stringify(s.border[cs])}`)}><button class="flex w-full items-center gap-2 px-4 py-3 text-[11px] text-red-500 transition-opacity hover:opacity-70">`);
      Log_out($$renderer2, { size: 13 });
      $$renderer2.push(`<!----> ${escape_html(logoutLabel)}</button></div></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--></div>`);
...
function _layout($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const queryClient = new QueryClient({
      defaultOptions: { queries: { staleTime: 60 * 1e3, retry: 1 } }
    });
    const bg = {
      light: "bg-gray-50 text-gray-800",
      dark: "bg-neutral-900 text-neutral-200"
    };
    $$renderer2.push(`<div${attr_class(`min-h-screen transition-colors duration-200 ${stringify(bg[colorSchema.mode])}`)}>`);
