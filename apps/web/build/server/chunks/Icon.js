...
async function customFetch(url, options) {
  ...
  if (response.status === 204) {
    return { data: void 0, status: 204, headers: response.headers };
  }
...
const authLogin = async (loginDto, options) => {
  return customFetch(
    getAuthLoginUrl(),
    {
      ...options,
...
const createAuthLogin = (options, queryClient) => {
  return createMutation(() => ({ ...getAuthLoginMutationOptions(options?.()) }));
};
const getAuthLogoutUrl = () => {
  return `/api/auth/logout`;
};
const authLogout = async (logoutDto, options) => {
  return customFetch(
    getAuthLogoutUrl(),
    {
      ...options,
...
const createAuthLogout = (options, queryClient) => {
  return createMutation(() => ({ ...getAuthLogoutMutationOptions(options?.()) }));
};
const getAuthSessionUrl = () => {
  return `/api/auth/session`;
};
const authSession = async (options) => {
  return customFetch(
    getAuthSessionUrl(),
    {
      ...options,
...
const getAuthSessionQueryKey = () => {
  return [
    `/api/auth/session`
  ];
};
const getAuthSessionQueryOptions = (options) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};
  const queryKey = queryOptions?.queryKey ?? getAuthSessionQueryKey();
  const queryFn = ({ signal }) => authSession({ signal, ...requestOptions });
  return { queryKey, queryFn, ...queryOptions };
};
function createAuthSession(options, queryClient) {
  const query = createQuery(() => getAuthSessionQueryOptions(options?.()));
  return query;
}
/**
...
export {
  ...
  skipToken as s,
  customFetch as t,
  createAuthSession as u,
  useQueryClient as v,
  createAuthLogout as w,
  getAuthSessionQueryKey as x,
  createAuthLogin as y,
  isApiError as z
};
