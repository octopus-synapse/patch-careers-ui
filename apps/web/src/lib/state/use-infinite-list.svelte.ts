...
export function useInfiniteList<T>(opts: UseInfiniteListOptions<T>) {
  ...
  async function loadMore(): Promise<void> {
    ...
    const firstPage = get(query).data;
  ...
  return {
    ...
    get hasMore(): boolean {
      ...
      // Use server's `hasNext` if extras haven't been loaded; otherwise
      // compare current pageNum vs totalPages.
      return pageNum < firstPage.totalPages;
    },
