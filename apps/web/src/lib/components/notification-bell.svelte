<script lang="ts">
	...
	async function handleNotificationClick(notification: Record<string, unknown>) {
		...
		<div class="overflow-y-auto flex-1">
			...
			{:else}
				{#each notifications as notification}
					{@const actor = (notification.actor ?? notification.sender ?? {}) as Record<string, string | null>}
					...
					<div
						...
						role="button"
						tabindex="0"
						class="flex items-start gap-2.5 px-4 py-3 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-neutral-700/50 {!notification.isRead ? 'bg-blue-50/50 dark:bg-neutral-800/30' : ''}"
					>
						<Avatar
						...
						<div class="min-w-0 flex-1">
							...
							<span class="text-[10px] text-gray-500 dark:text-neutral-500">
								{timeAgo((notification.createdAt ?? notification.created_at) as string)}
							</span>
						</div>
						{#if !notification.isRead}
