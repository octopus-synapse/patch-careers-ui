...
import {
  createSkillEndorsementsEndorse,
  createSkillEndorsementsGetSkills,
  createSkillEndorsementsWithdraw,
  getSkillEndorsementsGetSkillsQueryKey,
  skillEndorsementsGetEndorsers,
} from 'api-client';
...
const query = createSkillEndorsementsGetSkills(
  () => userId,
  () => ({ query: { enabled: browser && !!userId } }),
);

const skills = $derived.by<SkillSummary[]>(() => {
  const data = query.data as Record<string, unknown> | undefined;
  return (data?.skills as SkillSummary[] | undefined) ?? [];
});
...
const endorseMutation = createSkillEndorsementsEndorse(() => ({
  mutation: {
    onSuccess(_data, vars) {
      queryClient.invalidateQueries({ queryKey: getSkillEndorsementsGetSkillsQueryKey(userId) });
      track('skill_endorsed', { targetUserId: userId, skill: decodeURIComponent(vars.skill) });
    },
    onError(_err, vars) {
      const skill = decodeURIComponent(vars.skill);
...
const withdrawMutation = createSkillEndorsementsWithdraw(() => ({
  mutation: {
    onSuccess(_data, vars) {
      queryClient.invalidateQueries({ queryKey: getSkillEndorsementsGetSkillsQueryKey(userId) });
      track('skill_endorsement_withdrawn', {
        targetUserId: userId,
        skill: decodeURIComponent(vars.skill),
      });
    },
    onError(_err, vars) {
      const skill = decodeURIComponent(vars.skill);
...
function toggleEndorse(s: SkillSummary) {
  ...
  if (current.endorsedByMe) {
    ...
    withdrawMutation.mutate({ userId, skill: encodeURIComponent(s.skill) });
  } else {
    optimisticOverrides = {
      ...optimisticOverrides,
      [s.skill]: { endorsedByMe: true, delta: (optimisticOverrides[s.skill]?.delta ?? 0) + 1 },
    };
    endorseMutation.mutate({ userId, skill: encodeURIComponent(s.skill) });
  }
}

// Hover popover with endorsers
...
async function loadEndorsers(skill: string) {
  ...
  try {
    const res = (await skillEndorsementsGetEndorsers(userId, encodeURIComponent(skill), {
      page: 1,
      limit: 8,
    })) as unknown as Record<string, unknown> | undefined;
    const data = (res?.data as Endorser[] | undefined) ?? [];
				...
				>
					<button
						...
						class="group inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition-colors {eff.endorsedByMe
							...
							: 'border-gray-200 bg-gray-50 text-gray-700 dark:border-neutral-700 dark:bg-neutral-800/60 dark:text-neutral-300'} {isOwnProfile || !viewerAuthenticated
							? 'cursor-default'
