<script lang="ts">
  ...
  import { Button, Card, Loader, RankBadge, rankOf, toastState } from 'ui';
        ...
        <article class="flex flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <div class="relative aspect-[3/4] overflow-hidden border-b border-neutral-100 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-950">
            <object
              ...
              class="h-full w-full"
              aria-label={t('myProfile.styles.previewAria', { name: style.name })}
            >
            ...
            <div class="absolute left-2 top-2">
              <RankBadge
                score={style.styleScore}
                size="sm"
                showScore
                titleLabel={t('ui.matching.rankBadgeTitle', {
                  rank: rankOf(style.styleScore),
                  scoreSuffix: ` (${style.styleScore}/100)`,
                })}
              />
            </div>
            {#if style.isSystem}
