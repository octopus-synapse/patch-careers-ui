...
$effect(() => {
  if (!auth.isLoading && !authenticated) goto('/identity/sign-in');
});
						...
						<div class="p-5 space-y-4">
							<div>
								...
								<Input
									id="profile-name"
									type="text"
									bind:value={profileForm.values.name}
									class="mt-1"
								/>
							</div>
							<div>
								...
								<Textarea
									...
									class="mt-1"
								/>
								<p class="mt-1 text-right text-[10px] text-gray-500 dark:text-neutral-500">{(profileForm.values.bio ?? '').length}/500</p>
							</div>
