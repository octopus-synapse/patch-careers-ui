...
<div class="mx-auto max-w-5xl space-y-6 px-3 sm:px-6">
  ...
  {#if $jobsQuery.isLoading}
    <div class="grid gap-3">
      {#each Array(4) as _}
        <div
          class="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-800/50"
        >
  ...
  {:else}
    <div class="grid gap-3">
      {#each rows as job (job.id)}
        ...
        <article
          class="group flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5 transition-colors hover:border-gray-300 dark:border-neutral-800 dark:bg-neutral-800/50 dark:hover:border-neutral-700"
        >
          ...
          <div class="flex flex-wrap items-center gap-2 text-xs">
            ...
            {#if job.createdAt}
              <span class="inline-flex items-center gap-1 text-gray-400 dark:text-neutral-500">
                <Calendar size={12} />
                {t('company.jobs.postedOn').replace(
                  '{date}',
                  formatDate(job.createdAt, locale.current, 'long'),
                )}
              </span>
            {/if}
          </div>
          ...
          <div
            class="flex flex-wrap items-center justify-end gap-2 border-t border-gray-100 pt-3 dark:border-neutral-700/60"
          >
