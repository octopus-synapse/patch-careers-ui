 ...
 *
 * Recruiter describes the role (free-form JD + optional structured skills /
 * english / remote filters) and we rank up to 25 opt-in candidates by the
 * same FitScore used on the candidate side. Page consumes the Orval-generated
 * `recruitingMatchCandidates` mutation — no hand-written fetch.
 */
import {
...
let candidates = $state<MatchCandidatesDataDtoCandidatesItem[]>([]);
...
async function handleSearch() {
  ...
  submitting = true;
  try {
    const payload: MatchCandidatesRequestDto = {
			...
			<div class="space-y-3">
				{#each candidates as c (c.userId)}
					<article class="flex items-start gap-3 rounded-xl border border-gray-200 p-4 dark:border-neutral-700/60">
						<Avatar name={c.name ?? c.username ?? '?'} photoURL={c.photoURL ?? undefined} size="lg" />
						<div class="min-w-0 flex-1">
							<div class="flex flex-wrap items-center gap-2">
								...
								<FitScoreChip
									score={c.fit.score}
									matchedSkills={c.fit.breakdown.matchedSkills}
									missingSkills={c.fit.breakdown.missingSkills}
									englishMatch={c.fit.breakdown.englishMatch}
									remoteMatch={c.fit.breakdown.remoteMatch}
									labels={{
										match: 'match',
										title: 'Breakdown do fit',
										matchedHeader: 'Skills que batem',
										missingHeader: 'Faltando',
										englishLabel: 'Inglês',
										remoteLabel: 'Remoto',
										englishOk: 'Atende',
										englishBelow: 'Abaixo',
										englishUnknown: 'Sem dado',
										remoteExact: 'Alinhamento total',
										remotePartial: 'Parcial',
										remoteMismatch: 'Não bate',
										none: 'Nenhuma',
									}}
								/>
							</div>
