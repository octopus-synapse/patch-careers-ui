...
export const queryGenerator = createReactGenerator<PluginSvelteQuery>({
  ...
  Operation({ config, operation, generator, plugin }) {
    ...
    const isQuery =
      typeof options.query === 'boolean'
        ? true
        : options.query?.methods.some((method) => operation.method === method);
    const isMutation = difference(
      options.mutation ? options.mutation.methods : [],
      options.query ? options.query.methods : [],
    ).some((method) => operation.method === method);
    ...
    return (
      ...
      >
        ...
        {shouldUseClientPlugin && (
          <File.Import name={[client.name]} root={query.file.path} path={client.file.path} />
        )}
        {!shouldUseClientPlugin && (
          <File.Import
            name={['buildFormData']}
        ...
        {options.query && (
          <>
            ...
            <File.Import
              name={['QueryKey', 'QueryClient', 'CreateBaseQueryOptions', 'CreateQueryResult']}
              path={importPath}
              isTypeOnly
            />
            <Query
              ...
              typeSchemas={type.schemas}
              pathParamsType={options.pathParamsType}
              pathParamsAsGetters={options.pathParamsAsGetters}
              operation={operation}
              paramsCasing={options.paramsCasing}
              paramsType={options.paramsType}
              dataReturnType={options.client.dataReturnType || 'data'}
