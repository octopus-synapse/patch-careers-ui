/**
 * `BrandFace` — the brandmark with the mascot's face on it, straight from the
 * prototype's "a logo é o mascote": the two puzzle pieces with big round eyes
 * (pupils that follow the cursor on web) and a little smile.
 *
 ...
 *
 * Lives here rather than in the landing feature because the web navbar wears
 * it on every page — public and authenticated alike.
 */

import { brandPiecePalettes, landingBrandFacePalettes } from "@patch-careers/tokens";
...
export function BrandFace({ height = 54 }: BrandFaceProps): ReactElement {
  ...
  const pieces = brandPiecePalettes[theme];
  const face = landingBrandFacePalettes[theme];
