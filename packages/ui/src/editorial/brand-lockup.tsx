 ...
 * Tamagui `$tokens`.
 *
 * The PIECES come from `useBrandPieces()`, shared with the mascot — the logo
 * is the mascot, and this file's own dark pair (warm ivory + a lifted
 * periwinkle) had drifted until they no longer matched it.
 *
 * The LETTERFORMS keep their own tuning, because they are thin strokes rather
 * than solid slabs: warm ivory reads better than pure white on the dark bar,
 * and CAREERS gets a lighter blue to survive at that weight.
 */
...
// Letterform ink only — the pieces come from the shared brand palette.
const LIGHT = { ink: "#151A30", blueInk: "#5766E8" };
const DARK = { ink: "#E9E5D9", blueInk: "#8B96F5" };
...
export function BrandLockup({ height = 48 }: { height?: number }): ReactElement {
  ...
  const c = useThemeName() === "dark" ? DARK : LIGHT;
  return (
    <Svg
    ...
    >
      <Path d={PIECE_INK} fill={pieces.plain} />
