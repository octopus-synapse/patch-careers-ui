...
import Skeleton from '../skeleton/skeleton.component.svelte';
						...
						>
							{#if col.sortable}
								<Button
									...
									class="!px-0 !py-0 inline-flex items-center gap-1 font-semibold"
								>
									{col.label}
									{#if sortKey !== col.key}
						...
						<tr class="border-t border-gray-100 dark:border-neutral-800">
							{#each columns as col}
								<td class="px-3 py-2.5 sm:px-4 sm:py-3 {col.hideOnMobile ? 'hidden sm:table-cell' : ''}">
									<Skeleton shape="text" height="0.75rem" class="max-w-[120px]" />
								</td>
							{/each}
						</tr>
					{/each}
