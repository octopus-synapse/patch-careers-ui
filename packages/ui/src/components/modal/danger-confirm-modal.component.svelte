...
<Modal {open} {onClose}>
	{#snippet title()}
		<div class="flex items-start gap-3">
			...
			<div class="min-w-0 flex-1 pt-1">
				<h2 class="text-base font-semibold text-gray-900 dark:text-neutral-100">{titleText}</h2>
				<p class="mt-1 text-sm font-normal text-gray-600 dark:text-neutral-400">{message}</p>
			</div>
		</div>
	{/snippet}

	{#if warning}
		<div class="mb-4 rounded-md border border-red-200 bg-red-50 p-3 text-xs text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300">
			{warning}
		</div>
	{/if}
	...
	<div class="mt-4 flex flex-col-reverse gap-2 border-t border-gray-200 pt-3 dark:border-neutral-700 sm:flex-row sm:justify-end">
