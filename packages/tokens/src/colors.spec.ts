...
describe("palette", () => {
  it("exposes 5 ramps (gray/blue/red/green/amber)", () => {
    expect(Object.keys(palette).sort()).toEqual(["amber", "blue", "gray", "green", "red"]);
  });
...
describe("intent", () => {
  const names = ["neutral", "accent", "warn", "danger", "success"] as const;
  const variants = ["light", "dark"] as const;
  const slots = ["bg", "fg", "border", "hoverBg", "pressBg", "subtleBg", "subtleFg"] as const;

  it("exposes 5 intent names", () => {
